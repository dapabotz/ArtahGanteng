...
try {
		...
		const ownerNumberr = ["6285784786722@s.whatsapp.net","6285784786722@s.whatsapp.net","6285784786722@s.whatsapp.net","6285784786722@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*💠ArtahZyx💠*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Kw69Oel34Nd0JuluvFNVKt', "groupName": `${NamaBot}`, "footerText": "*💠ArtahZyx💠*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Sory sepertinya  ${NamaOwner} Lagi  Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nWa nanti aja Majikan Gw belum Online`, "*💠ArtahZyx💠*", [{buttonId: 'simi', buttonText: {displayText: 'Oke Artah Ganteng'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
				...
				for (let ment of mek.message.extendedTextMessage.contextInfo.mentionedJid) {
					if (ment === denz.user.jid){
                        ...
                        addafk(mek.key.remoteJid)
						heheh = ms(Date.now() - waktuafk)
			       sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "💠ArtahZyx💠", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
				}
						}
					}
   ...
   const sotoy = [
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
      ...
      if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
       if (args.length < 1) return reply(`Penggunaan ${prefix}setmenu butloc/butlist`)
           if(args[0] == 'butloc'){
                      modelmenu = "butloc"
                      reply('_Success Changed To Button Loc Menu_')
               } else if(args[0] == 'butlist'){
                      modelmenu = "butlist"
                      reply('_Success Changed To Button List Menu_')
               } else {
        ...
        case 'menu':
        case 'help':
        ptod = "6285784786722@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
...
${uwu}`
       const fduct = {
					"productMessage": {
						"product": {
							"productImage":{
							"url": "https://mmg.whatsapp.net/d/f/Ai7QsXsbOycIm_TfJFWn5PxJyHWBdyaRHZ9gXNeFvySP.enc",
							"mimetype": "image/jpeg",
							"fileSha256": "YJ4KSwWSDFflUWG8mytKV+5NVaXGna5ApmwbIYp3aoE=",
							"fileLength": "99999999999999999",
							"height": 640,
							"width": 640,
							"mediaKey": "6GKKq3boxk24ahwVlkd6nq0hRw+cjTiB9SaLRKJ1KrM=",
							"fileEncSha256": "zTBIjca1a3tUCLtqC4yQU3GTdT0E9N94J5/CLf9FS7c=",
						...
						"businessOwnerJid": "6285784786722@s.whatsapp.net"
					}
				}
emsce = denz.prepareMessageFromContent(from, fduct, { quoted: ftex })
            denz.relayWAMessage(emsce)
break
case 'command':
 listMsg = {
 buttonText: 'LIST MENU',
...
client.browserDescription = ['Helga Ilham','Desktop','3.0']
...
break
		...
		case 'sc':
		case 'sourcecode':
		denz.sendMessage(from, { text: "Wa.me/6285784786722", matchedText: 'Wa.me/6285784786722', description: "", title: "don't click here !!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
...
break
            ...
            try {
                axios.get(`https://api.zeks.me/api/yts?apikey=ramaganzapikey&q=${body.slice(10)}`).then((xres) =>{
			if (xres.status !== true)
			secs = []
                        ...
                        "rows": [
                           {
                              ...
                              "rowId": `.ytmp3 ${xres.video.url}`
                           },
						   {
                              "title": "MP4",
							  description: `Title: ${xres.video.title}\n\nUploader: ${xres.uploader.username}`,
                              "rowId": `.ytmp4 ${xres.video.url}`
                           }
                        ], title: i+1})
			})
			let po = denz.prepareMessageFromContent(from, {
				  "listMessage":{
                  "title": "*Silahkan Pilih!*",
                  "description": `*Query: ${body.slice(10)}*\n*Select*`,
                  "buttonText": "Result",
                  "listType": "SINGLE_SELECT",
                  "sections": secs}}, {}) 
            denz.relayWAMessage(po)
            })
            } catch (err) {
							reply(mess.error.api)
						}
